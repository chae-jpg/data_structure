...
// input: root node, level of search
// output: m_LUT, m_bit_size
void huffman_traversal(TreeNode* node, TreeNode* parent, int level)
{
	//if node is left child of parent -> add 0
	if (node == parent->l) {
		//copy bits from parent
		for (int i = 0; i < level; i++) {
			node->bits[i] = parent->bits[i];
		}
		node->bits[level] = 0;
	}
	//if node is right child of parent -> add 1
	else {
		//copy bits from parent
		for (int i = 0; i < level; i++) {
			node->bits[i] = parent->bits[i];
	...
	//set node's size : +1 than parent's size
	node->bit_size = parent->bit_size + 1;

	//if node has data
	if (node->data != NULL) {
		int idx = node->data - 'a';
		//set m_LUT, m_bit_size same as node
		m_LUT[idx] = node->bits;
	...
	huffman_traversal(node->l, node, level + 1);
	huffman_traversal(node->r, node, level + 1);
}
...
void huffman_encoding(char* str, bits_stream* bits_str)
{
	...
	for (int i = 0; str[i] != NULL; i++) {
		for (int j = 0; j < m_bit_size[str[i]-'a']; j++) {
			bits_str->stream[idx++] = m_LUT[str[i] - 'a'][j];
			bits_str->length++;
		}
	}
...
int huffman_decoding(bits_stream* bits_str, TreeNode* node, char* decoded_str)
{
	int idx = 0, d_i = 0;
	TreeNode* cur = node;
	while (idx <= bits_str->length) {
		if ((!bits_str->stream[idx] && cur->l == NULL) || (bits_str->stream[idx] && cur->r == NULL)) {
			decoded_str[d_i++] = cur->data;
			cur = node;
			continue;
		}
		if (!bits_str->stream[idx]) {
			cur = cur->l;
			idx++;
		}
		else {
...
int main()
{
	...
	//to set the bit_size of element_root.ptree->bit_size to 0
	element_root.ptree->bit_size = -1;
	// Generate the huffman codeword from the huffman binary tree
	huffman_traversal(element_root.ptree, element_root.ptree, -1);
