...
void main()
{
	...
	while (clock < duration) {
		...
		if (service_time_1 > 0)	// the customer is receiving service
			service_time_1--;
		...
		// So, take out a customer from the queue and start the service.
			...
			service_time_1 = remove_customer();
			if (service_time_1) {
				printf("Staff 1 accepted customer.\n");
			}
		}
		...
		if (service_time_2 > 0)
			...
			service_time_2 = remove_customer();
			if (service_time_2) {
				printf("Staff 2 accepted customer.\n");
			}
		}
	}
	print_stat();
